...
namespace Servicios.ServMatricula
{
    public class SvMatricula: ISvMatricula
    {
        ...
        public List<Matricula> GetAllMatriculaPorCarrera_Nivel(int IdCarrera, int IdNivel)
        {
            return _myDbcontext.Matriculas.Where(x => x.CarreraId == IdCarrera ).Where(x=>x.NivelId == IdNivel).Include(x => x.Carrera).Include(x=>x.Nivel).ToList();
        }
